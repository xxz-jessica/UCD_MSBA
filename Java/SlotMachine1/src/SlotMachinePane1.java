...
public class SlotMachinePane1 extends VBox{
	...
	//-------------------------------
	public SlotMachinePane1 () {
		count = 5;//count is the token that the user has
		value = 0;//value is the slot combinations' value
		...
		imageLabel = new Image ("aaa.png");//slot combinations
	...
	public void processButtonPress (ActionEvent event) {
		...
		line = gen.nextInt(27);//randomly generate a number (0-26)for the switch statement
		if (count>=0)//if the count is not negative
		{
			if (event.getSource() == spinButton) 
			{
				...
				tokenLabel.setText("Current token: "+ count);
				if (count>0)
					textLabel.setText("Your cash out value is "+count+((count!=1)?" tokens":" token"));
				else
		...
		else//if the count less than 0
		{
			int res=JOptionPane.showConfirmDialog(null,"Do you want to play again?", "choose one",JOptionPane.YES_NO_OPTION);
		 	if(res==JOptionPane.YES_OPTION){
		 		...
		 		tokenLabel.setText("Current token: "+ count);
		 		spinLabel.setText("Result of spin: "+ value);
		 		textLabel.setText("No Button Was Pushed");
		 	}
		 	else{
		 		System.exit(0);
		 		return;
		...
		if (event.getSource() == cashButton) //if the user wants to cash out
		{
			int res=JOptionPane.showConfirmDialog(null,"Your result is "+ count+" ."+" Do you want to play again?", "choose one",JOptionPane.YES_NO_OPTION);
		 	if(res==JOptionPane.YES_OPTION)
		 	{
		 		count = 5;
		 		value = 0;
